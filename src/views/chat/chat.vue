  ...
  <div class="chat-body">
    ...
    <ul class="chat-box">
      <li
        ...
        :class="[item.type == 'reply' ? 'chat-ai-box' : 'chat-user-box']"
        :key="item.type"
      >
        <div class="item-content">
          ...
          <div class="text-box" style="white-space: pre-wrap">
            <span>{{ item.text }}</span>
            <div class="cursor" v-if="load && chatData.length - 1 == index"></div>
          </div>
        </div>
      </li>
    </ul>
    ...
    <div class="chat-input">
      ...
      <span>
        ChatGPT Feb 13 Version. Free Research Preview. Our goal is to make AI systems more natural
...
<script setup lang="ts">
  ...
  const getReply = async (question: string) => {
    // 请求接口 获取答案
    let chatAnswer = await getChatResponse(question)

    //如果返回的参数为空
    if (!chatAnswer) {
      typewriter('抱歉，服务器好像挂掉了，请刷新重试。')
      return
    }
    ...
    if (typeof chatAnswer != 'string') {
      ...
      // 将错误提示写入聊天列表
      typewriter('请求可能超时，请刷新页面或换个问题重试')
      return
    }
    // 去除返回字符串头部的两个无用换行符
    chatAnswer = await formatString(chatAnswer)
    typewriter(chatAnswer)
  }
  ...
  function typewriter(text: string) {
    if (i.value <= text.length) {
      ...
      chatData[chatData.length - 1].text = text.slice(0, i.value++)
      timer = setTimeout(() => {
        typewriter(text)
      }, 10)
    } else {
...
<style lang="scss" scoped>
  .chat-body {
    ...
    .chat-input {
      ...
      align-items: center;
      justify-content: center;
      background-image: linear-gradient(to bottom, #ffffffa8, #fff, #fff, #fff, #fff);
      box-shadow: #fff 0 0 30px 0px;
      .input {
        position: relative;
        max-width: 800px;
        width: 100%;
